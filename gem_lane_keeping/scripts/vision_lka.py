...
- Primary: detect ONLY the solid YELLOW left border and drive parallel to it with a fixed rightward offset.
- Bootstrap: if left yellow is missing (e.g., at start), temporarily use WHITE ONLY in a left-side band,
             ignoring the middle dashed center. Drive slower until yellow is confidently found.

Control:
- Lateral error = (desired_x - image_center_x) normalized to [-1,1].
- Heading = slope (atan) of the left line (yellow if available; white-band in bootstrap).
- PD(+heading) steering with rate limit and EMA; speed profile with lateral/dvdt limits.
"""
...
class VisionLKA:
    def __init__(self):
        ...
        self.roi_top_boot   = rospy.get_param("~roi_top_boot", 0.55)  # lower ROI in bootstrap
        ...
        # --- bootstrap (white fallback limited to the left-side band) ---
        self.bootstrap_enable          = rospy.get_param("~bootstrap_enable", True)
        self.bootstrap_frames_confirm  = rospy.get_param("~bootstrap_frames_confirm", 8)   # yellow must be good N frames
        self.bootstrap_speed_scale     = rospy.get_param("~bootstrap_speed_scale", 0.75)  # drive slower
        self.left_band_frac            = rospy.get_param("~left_band_frac", 0.35)         # use white in left X% of width
        self.center_ignore_frac        = rospy.get_param("~center_ignore_frac", 0.20)     # cut middle to avoid dashed
        self.yellow_good_px_threshold  = rospy.get_param("~yellow_good_px_threshold", 300)

        # --- color thresholds (robust yellow; slightly wider on startup) ---
    ...
    def on_image(self, msg: Image):
        ...
        if have_yellow or not self.bootstrap_active:
            left_xs, used_y, src = left_xs_yel, used_y_yel, "Y"
            edges_dbg = mask_yellow
        else:
            # bootstrap: use WHITE only in a left-side band, ignore center
        ...
        # speed profile (+ slower in bootstrap)
    ...
    def _yellow_mask(self, roi_bgr):
        # mild contrast boost helps yellow
    ...
    def _left_white_band(self, mask_white, W):
        band = int(W * float(self.left_band_frac))
        cen  = int(W * float(self.center_ignore_frac))
        out = np.zeros_like(mask_white)
        out[:, :band] = mask_white[:, :band]        # left-side only
        cL, cR = (W//2 - cen), (W//2 + cen)        # remove center entirely
        cL = max(0, cL); cR = min(W, cR)
        if cR > cL: out[:, cL:cR] = 0
        return out
    ...
    def _draw_debug(self, roi_bgr, edges, left_xs, used_y, desired_x, W_full, src, boot):
        ...
        cv2.line(mix, (int(desired_x), mix.shape[0]-5), (int(desired_x), mix.shape[0]-55), (0,255,255), 2)
        tag = "BOOT" if boot else "YEL"
        cv2.putText(mix, f"{tag}/{src}", (10, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,255), 2, cv2.LINE_AA)
        cv2.putText(mix, f"delta={self.steer_filt:+.2f} v={getattr(self.last_cmd,'speed',0.0):.2f}",
                    (10, mix.shape[0]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1, cv2.LINE_AA)
        return mix
